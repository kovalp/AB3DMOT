...
from ab3dmot_io.core.utils import find_all_frames, get_subfolder_seq, load_config
...
def combine_trk_cat(split: str, dataset: str, method: str, config_path: Path, save_root: str) -> None:
	...
	log = os.path.join(root_dir, '%s_%s' % (method, split), 'combine_log.txt')
	mkdir_if_missing(log); log = open(log, 'w+')

	# source directory
	subset = ['%s_%s_%s' % (method, cat, split) for cat in cfg.cat_list]

	# loop through all hypotheses
	for hypo_index in range(1):
		data_suffix = '_%d' % hypo_index
		frame_dict = find_all_frames(root_dir, subset, data_suffix, seq_list)

		############ merge for 3D MOT evaluation
		save_root = os.path.join(root_dir, '%s_%s' % (method, split), 'data'+data_suffix);
